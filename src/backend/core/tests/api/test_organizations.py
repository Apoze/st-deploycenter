...
def _setup_org(user, superuser=False, **org_kwargs):
    """Create a user (optionally superuser) with access to a new org."""
    operator = factories.OperatorFactory()
    factories.UserOperatorRoleFactory(user=user, operator=operator)
...
def test_api_organization_proconnect_domains_superuser_updates_manual_only():
    ...
    response = client.patch(
        ...
        {"manual": ["New.FR ", "b.fr", "b.fr", "invalid"]},
        format="json",
    )
    assert response.status_code == 200
    # normalized: lowercased, trimmed, deduped, "." required, sorted
...
def test_api_organization_proconnect_domains_change_creates_log_entry():
    ...
    entry = LogEntry.objects.filter(object_id=str(organization.pk)).latest("action_time")
...
def test_api_organization_superuser_can_discard():
    ...
    assert response.status_code == 200
    assert response.json() == _full(candidates=["keep.fr", "slug.fr"], discarded=["slug.fr"])
...
def test_api_organization_regular_user_cannot_discard():
    ...
    client = APIClient()
    client.force_login(user)
    operator, organization = _setup_org(user, proconnect_domains={"candidates": ["slug.fr"]})
...
def test_api_organization_proconnect_domains_prevalidated_intersection():
    """_prevalidated is the org's domains already in the deployed allowlist."""
    cache.clear()
    user = factories.UserFactory()
    client = APIClient()
    client.force_login(user)
    ...
    store_prevalidated_fqdns("idp-x", ["a.fr", "c.fr", "unrelated.fr"])

    response = client.get(
        f"/api/v1.0/operators/{operator.id}/organizations/{organization.id}/"
    )
    assert response.status_code == 200
    pd = response.json()["proconnect_domains"]
    # Per-idp: a.fr/c.fr are deployed on idp-x; pending.fr isn't; unrelated.fr
    # isn't the org's.
    assert set(pd["_prevalidated"]["idp-x"]) == {"a.fr", "c.fr"}
    cache.clear()


def test_api_organization_proconnect_domains_prevalidation_unknown():
    """No _prevalidated key when the deployed allowlist is not cached."""
    cache.clear()
    user = factories.UserFactory()
    client = APIClient()
    client.force_login(user)
    ...
    assert response.status_code == 200
    assert "_prevalidated" not in response.json()["proconnect_domains"]


def test_api_organization_proconnect_domains_prevalidated_empty_but_defined():
    """An empty-but-defined allowlist yields _prevalidated: [] (all 'not yet')."""
    cache.clear()
    user = factories.UserFactory()
    client = APIClient()
    client.force_login(user)
    ...
    store_prevalidated_fqdns("idp-x", [])  # defined, but empty
    ...
    assert response.json()["proconnect_domains"]["_prevalidated"] == {"idp-x": []}
    cache.clear()
...
def test_api_organization_proconnect_domains_prevalidated_is_per_idp():
    ...
    for idp in ("idp-a", "idp-b"):
        ...
        factories.OperatorServiceConfigFactory(operator=operator, service=service)
    store_prevalidated_fqdns("idp-a", ["a.fr"])  # deployed on idp-a
    store_prevalidated_fqdns("idp-b", [])  # NOT on idp-b

    response = client.get(
        f"/api/v1.0/operators/{operator.id}/organizations/{organization.id}/"
    )
    prevalidated = response.json()["proconnect_domains"]["_prevalidated"]
